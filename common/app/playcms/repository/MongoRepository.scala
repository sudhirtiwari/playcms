...
abstract class MongoRepository[T <: Model[ID], ID](db: DefaultDB)
                                                  (implicit val ec: ExecutionContext,
                                                   val format: Format[T],
                                                   val idWrites: Writes[ID]) {
  val collectionName: String
  val collection = db.collection[JSONCollection](collectionName)

  def findById(id: ID) = findOne(Json.obj("_id" -> id))
  def findAll = collection.find(Json.obj()).cursor[T].collect[List]()
  ...
  def delete(id: ID) = collection.remove(Json.obj("_id" -> id)) map (_ => {})
}

trait NaturalKeyMongoRepository[T <: Model[ID], ID] { this: MongoRepository[T, ID] =>
